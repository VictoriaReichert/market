<?php // Получаем список ярмарок, на которых пользватель $email является продавцом
    $userLogin = $userData['login'];
    $tableName = "temp.`".$userLogin."_sel`";
    ...
    while($marketDBName = mysqli_fetch_assoc($dbNames)):
        ...
        $s = mysqli_query($connection, "SELECT * FROM $n.`market-members` WHERE `role` = 'seller' AND email = '$email' AND `status` = 'Участвует'");
